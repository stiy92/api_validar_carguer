...
import { Con_veh } from "../interfaces/interface";
...
ConfivehiRoutes.post('/configuracion_veh',  async (req: Request, res: Response) =>{
    ...
    <soap:Body>
    <Cargar_Configuracion_Vehicular xmlns="http://tempuri.org/">
        <Usuario>${Codigo}</Usuario>
        <Clave>${hashedClave}</Clave>
      </Cargar_Configuracion_Vehicular>
        </soap:Body>
      </soap:Envelope>`;
        ...
        const respuest = await axios.post(url, xmlBody, {
            headers: {
                ...
                SOAPAction: 'http://tempuri.org/Cargar_Configuracion_Vehicular',
              },

        });

         //varalbe de tipo arreglo para agregar los datos como los quiero en mi interface map
         let configuracionv: Con_veh[] = [];
       ...
       xmlparser.parseString(respuest.data, (err:any, result:any)=>{
        ...
        const informacionMovilGeneral = result['soap:Envelope']['soap:Body']['Cargar_Configuracion_VehicularResponse']['Cargar_Configuracion_VehicularResult']['Informacion_Movil_General'];
        ...
        if (Array.isArray(informacionMovilGeneral)) {
            ...
            informacionMovilGeneral.forEach((info: any) => {
                ...
                configuracionv.push({ Codigo: codigo, Descripcion: descripcion });
            });
     ...
     if (respuest.data) {
        ...
        // respuestaSOAP: respuest.data, 
           ...
           export default ConfivehiRoutes;