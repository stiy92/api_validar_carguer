...
import { Empaques } from "../interfaces/interface";
...
EmpaquesRoutes.post('/empaques',  async (req: Request, res: Response) =>{
    ...
    <soap:Body>
    <Cargar_Empaques xmlns="http://tempuri.org/">
       ...
       xmlparser.parseString(respuest.data, (err:any, result:any)=>{
        ...
        const informacionMovilGeneral = result['soap:Envelope']['soap:Body']['Cargar_EmpaquesResponse']['Cargar_EmpaquesResult']['Informacion_Movil_General'];
        ...
        if (Array.isArray(informacionMovilGeneral)) {
            ...
            informacionMovilGeneral.forEach((info: any) => {
                ...
                empaques.push({ Codigo: codigo, Descripcion: descripcion });
            });
